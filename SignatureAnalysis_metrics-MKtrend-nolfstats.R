...
ws_040500MI_hd <- read.csv(file = "~/OneDrive - University of Kansas/Documents/Powell/DiscoverStreams/outputs/watershed_info/ws_040500MI_hd_1960-2021.csv", header = TRUE, row.names = 1)
...
for (i in 1:nrow(ws_110300KS)) {
      site_number <- paste(0, ws_110300KS$site_no[i], sep = "")
...
for (i in 1:nrow(ws_180102CA)) {
  ...
  ## CALCULATE RECESSION
  for(y in seq_along(sf)){
    y = 1
    currentyear <- as.numeric(sf$year[y])
