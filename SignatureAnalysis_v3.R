...
colnames(sf_metrics_year_MI_ep) <- "Year"
sf_metrics_year_KS_ep <- data.frame(seq(1902, 1940, by = 1))
colnames(sf_metrics_year_KS_ep) <- "Year"
sf_metrics_year_CA_ep <- data.frame(seq(1904, 1940, by = 1))
colnames(sf_metrics_year_CA_ep) <- "Year"
...
# for (i in 23:nrow(ws_040500MI)) {
#         site_number <- ws_040500MI$site_no[i] ## failed at 6, 8, 9, 10, 11, 14, 18, 19, 22
for (i in 1:nrow(ws_110300KS)) {
      site_number <- ws_110300KS$site_no[i]  ## failed at 3,
...
#   site_number <- ws_180102CA$site_no[i]
  ...
  mk_test_Q10 <- trend::mk.test(sf_metrics$Q10)
  mk_test_Q50 <- trend::mk.test(sf_metrics$Q50)
  mk_test_Q90 <- trend::mk.test(sf_metrics$Q90)
  mk_test_Q95 <- trend::mk.test(sf_metrics$Q95)
  ...
  # write.csv(sf_metrics, paste("~/GradSchool/DiscoverStreams/outputs/sfmetrics_MI/MI__ep/MI_", i, "_metrics_ep.csv"))
...
p_metrics_melt <- ggplot2::ggplot(sf_metrics_melt, aes(x = Year, y = Discharge, color = Metric, linetype = Metric, size = Metric)) +
    geom_ribbon(data = sf_metrics, mapping = aes(x = hyear, ymin = Q10, ymax = Q90), fill = "grey70", inherit.aes = FALSE) +
    geom_line() +
