...
for(i in seq_along(usgs.files)){
  ...
  currentBudyko <- data.frame(matrix(ncol=8,nrow=0))
  colnames(currentBudyko) <-c("siteID", "interval", "group", "meanSF", "meanP", "meanPET", "PET.P", "AET.P")
  currentBudyko[1,] <- c(currentsite, "NA" , currentgroup, meanSF, meanP ,meanPET,  PET.P, AET.P)
  # erroneous 
  # currentBudyko[1,] <- c(currentsite, "NA" , currentgroup, meanP ,meanPET, meanSF, PET.P, AET.P)
