...
for(i in seq_along(SiteIds)){
  ...
  #current$climyear <- current$year
  current$climyear <- ifelse(current$month<4, current$year-1,current$year)
  current <- subset(current, (current$climyear >= StartYear) & (current$climyear <= EndYear))
  ...
  current$interval <- round_any(current$climyear, IntervalLen, f=floor)
  ...
  current_P$month <- month(current_P$Date)
  #current_P$climyear <- current_P$year
  current_P$climyear <- ifelse(current_P$month<4, current_P$year-1,current_P$year)
  ...
  current_PET$month <- month(current_PET$Date)
  #current_PET$climyear <- current_PET$year
  current_PET$climyear <- ifelse(current_PET$month<4, current_PET$year-1,current_PET$year)
  current_PET$interval <- round_any(current_PET$climyear, IntervalLen , f=floor)
  current_PET <- subset(current_PET, (current_PET$climyear>=StartYear) & (current_PET$climyear<=EndYear))
